...
export class CanvasSizesApp extends CanvasApp {
    ...
    initCanvas() {
        ...
        this.canvas.addEventListener(MouseEvent.DOWN, this.onCanvasClick.bind(this));
    ...
    onCanvasClick(type, event) {
        this.createBall({x: event.x, y: event.y, radius: 5, color: new Color(200, 200, 200)});
    }

    // MARK: - helpers
    updateBallPositions() {
        this.#topLeftBall.position = new Vec2(50, 50);
        this.#topRightBall.position = new Vec2(this.canvas.getWidth() - 50, 50);
        this.#bottomRightBall.position = new Vec2(this.canvas.getWidth() - 50, this.canvas.getHeight() - 50);
        this.#bottomLeftBall.position = new Vec2(50, this.canvas.getHeight() - 50);
        this.#centerBall.position = new Vec2(this.canvas.getWidth() / 2, this.canvas.getHeight() / 2);
    }
