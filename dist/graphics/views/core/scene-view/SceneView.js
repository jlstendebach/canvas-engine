...
export class SceneView extends View {
    ...
    scaleAround(factor, anchorX, anchorY, coordinateSpace = CoordinateSpace.LOCAL) {
        ...
        if (coordinateSpace === CoordinateSpace.CONTENT) {
            ...
            dx -= anchor.x;
            dy -= anchor.y;
            this.#contentView.transform.setPositionXY(anchor.x + (dx * factor), anchor.y + (dy * factor));
        }
        else if (coordinateSpace === CoordinateSpace.LOCAL) {
            dx -= anchorX;
            dy -= anchorY;
            this.#contentView.transform.setPositionXY(anchorX + (dx * factor), anchorY + (dy * factor));
        }
    ...
    rotateAround(radians, pivotX, pivotY, coordinateSpace = CoordinateSpace.LOCAL) {
        ...
        if (coordinateSpace === CoordinateSpace.CONTENT) {
            ...
            dx -= localPivot.x;
            dy -= localPivot.y;
            this.#contentView.transform.setPositionXY(localPivot.x + (dx * cos) - (dy * sin), localPivot.y + (dx * sin) + (dy * cos));
        }
        else if (coordinateSpace === CoordinateSpace.LOCAL) {
            dx -= pivotX;
            dy -= pivotY;
            this.#contentView.transform.setPositionXY(pivotX + (dx * cos) - (dy * sin), pivotY + (dx * sin) + (dy * cos));
        }
