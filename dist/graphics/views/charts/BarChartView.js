...
export class BarChartView extends View {
    ...
    createAndMeasureLeftLabels(context) {
        const offset = 10;
        const labelWidth = this.getWidth() / 3
            - this.padding.left
            - this.padding.right
            - offset;
        let maxWidth = 0;
    ...
    layoutBars() {
        ...
        const barHeight = Math.floor((this.graphArea.y2
            - this.graphArea.y1
            - (this.barData.count() + 1) * barPadding) / this.barData.count());
        const barWidthMax = this.graphArea.x2 - this.graphArea.x1;
        const totalBarHeight = (barHeight + barPadding) * this.barData.count()
            - barPadding;
        const top = this.graphArea.y1 + Math.floor((this.graphArea.y2
            - this.graphArea.y1
            - totalBarHeight) / 2);
        this.barViews = [];
        for (let i = 0; i < this.barData.count(); ++i) {
    ...
    layoutLeftLabels(context) {
        for (let i = 0; i < this.leftLabels.length; ++i) {
            ...
            if (label.getHeight() > bar.getHeight()) {
                ...
                label.setHeight(Math.max(lineHeight, Math.floor(bar.getHeight() / lineHeight) * lineHeight));
            }
        }
    }
    /*
    ...
    */
    ...
    drawBottomLabels(context) {
        ...
        for (let i = 0; i < this.bottomLabels.length; i += 2) {
            ...
            let w = (i < this.bottomLabels.length - 1
                ...
                : this.graphArea.x2 - x);
            context.rect(x, this.graphArea.y1, w, this.graphArea.y2 - this.graphArea.y1);
        }
    ...
    onDraw(context) {
        ...
        */
        ...
        context.fillStyle = "white";
        context.beginPath();
        context.rect(this.graphArea.x1, this.graphArea.y1, this.graphArea.x2 - this.graphArea.x1, this.graphArea.y2 - this.graphArea.y1);
        context.stroke();
        context.fill();
        // Draw components    
    ...
    onMouseDrag(event) {
        ...
        let vecCurrent = new Vec2(event.x - this.getWidth() / 2, event.y - this.getHeight() / 2);
        let vecPrevious = new Vec2(vecCurrent.x - event.dx, vecCurrent.y - event.dy);
        let vecDiff = new Vec2(Math.abs(vecCurrent.x) - Math.abs(vecPrevious.x), Math.abs(vecCurrent.y) - Math.abs(vecPrevious.y));
