export class Color {
    ...
    static fromString(rgba) {
        ...
        color.setRGBA(parseInt(values[0]), parseInt(values[1]), parseInt(values[2]), parseInt(values[3] * 255));
        return color;
    }
    static copy(other) {
        let color = new Color();
    ...
    static hexString(r, g, b, a = 255) {
        return new Color(r, g, b, a).toHexString();
    }
    // -------------------------------------------------------------------------
    setRGBA(r, g, b, a) {
        this.#value = ((r << Color.#RED_BITS) & Color.#RED_MASK
            | (g << Color.#GREEN_BITS) & Color.#GREEN_MASK
            | (b << Color.#BLUE_BITS) & Color.#BLUE_MASK
            | (a << Color.#ALPHA_BITS) & Color.#ALPHA_MASK) >>> 0;
    }
    setRGB(r, g, b) {
    ...
    // --[ operations ]---------------------------------------------------------
    add(c) {
        this.setRGBA(Math.max(0, Math.min(255, this.r + c.r)), Math.max(0, Math.min(255, this.g + c.g)), Math.max(0, Math.min(255, this.b + c.b)), Math.max(0, Math.min(255, this.a + c.a)));
        return this;
    }
    // --[ helpers ]------------------------------------------------------------
    /**
...
//# sourceMappingURL=OldColor.js.map