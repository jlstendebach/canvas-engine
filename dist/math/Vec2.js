...
export class Vec2 {
     ...
     * @returns {Vec2} This vector.
     */
    limitLength(max, current = this.length()) {
        if (current > max) {
     ...
     * @returns {Vec2} This vector.
     */
    clampLength(min, max = null, current = this.length()) {
        if (current < min) {
    ...
    static fromArray(array, offset = 0) {
        try {
            return new Vec2(parseFloat(array[offset]) || 0, // x 
    ...
    static fromObject(object) {
        try {
            return new Vec2(parseFloat(object.x) || 0, // x
    ...
    static reflect(v, normal) {
        ...
        return new Vec2(v.x - normal.x * scale, v.y - normal.y * scale);
    }
    static mirror(v, mirror) {
        const scale = 2 * v.dot(mirror);
        return new Vec2(mirror.x * scale - v.x, mirror.y * scale - v.y);
    }
    static lerp(v1, v2, a) {
        return new Vec2((1 - a) * v1.x + a * v2.x, (1 - a) * v1.y + a * v2.y);
    }
    static projection(v1, v2) {
        const scale = v1.dot(v2) / v2.lengthSq();
        return new Vec2(v2.x * scale, v2.y * scale);
    }
    ...
    static unitNormal(v, length = v.length()) {
        return new Vec2(-v.y / length, v.x / length);
    }
    static dot(v1, v2) {
