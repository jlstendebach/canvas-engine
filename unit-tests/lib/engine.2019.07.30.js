...
class EventEmitter {
  ...
  remove(type, callback, owner) {
    var listenerList = this.listeners[type];
    if (typeof listenerList !== "undefined") {
      for (var i=0; i<listenerList.length; i++) {
        if (listenerList[i].callback === callback &&
            listenerList[i].owner === owner) {
...
class LineStringView extends View { 
  ...
  getX() { return this.vertices[0].x; }
  getY() { return this.vertices[0].y; }
...
class VectorView extends View {
  ...
  drawArrow(context) {
    ...
    const normal = Vec2.normal(this.vector).setMagnitude(arrowWidth/2);
    const inverse = Vec2.invert(this.vector).setMagnitude(arrowHeight);
  ...
  drawLine(context, arrowHeight) {
    ...
    const scaled = Vec2.copy(this.vector).setMagnitude(newMag);
